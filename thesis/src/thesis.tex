...
\paragraph{RQ1: How do we create a merge tool for evolution plans that respect soundness?}%
\label{par:rq1_bla_bla}

\todo{As soundness is important for ensuring successful long-term evolution, we wanted the tool to guarantee soundness. We did this by .....}

\paragraph{RQ2: How do we design an algorithm for merging evolution plans?}%
\label{par:rq2}

\todo{As specified in the outline for the three-way merge algorithm in Section~...., the algorithm were split up in distinct steps. The steps transformed the inputs one step at a time in order to get the results we wanted.}

\paragraph{RQ3: Are the results of the merge tool predictable and interpretable?}%
\label{par:rq3}

\todo{An important aspect to merging is that actual engineers has to interpret and act on the results of the merge. For this reason, we wanted to create results that made sense. We created an incremental merger, merging one time point at a time, which allows the engineer to step through the resulting plan to ensure a satisfactory result. In cases of failure, we wanted to report the reason for failure, such that the engineer could find and fix the cause of the error.}
