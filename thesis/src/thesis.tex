...
\todo{Next step is finding a way of representing changes between two feature models to explicitly model the changes.}

\todo{what we did in the paper was very similar. However, even though such a semantic might work, the representation has several issues we will shed some light on and discuss. In creating a better representation, it is beneficial to know some pitfalls we want to avoid.}

\todo{what we did previously. defined operations like addFeature, moveGroup and changeName.} 

\todo{want to avoid several ways of representing the same type of change. if two vers ns have identical changes, we dont care how those changes can be done. doing one thing first then another. example.}
