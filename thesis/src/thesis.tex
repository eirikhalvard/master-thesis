...
Part of our goal is to merge sound evolution plans and produce an evolution plan that is sound. An important prerequisite in order to achieve this, is to formally define evolution plans.

Part of our work on this project was defining the formal semantics of feature models and evolution plans. The development of the formal semantics can be read in greater detail in our paper \cite{cite:consistency_preserving_evolution_planning}. However, we present the details necessary in order to understand the formal semantics of evolution plans.

\subsection{Feature Models}%
\label{sub:feature_models}

Since evolution plans consist of feature models, we start by defining feature models formally. \textit{Feature models} are a recursive tree-structure of \textit{features} and \textit{groups}. Each feature model begins with a \textit{root feature}, which has an arbitrary number of child groups. Each group has in turn an arbitrary number of features, etc. Each feature and group has also an unique id. Each feature consists of an id, name and a type. Each features type can either be \textit{mandatory} or \textit{optional}. Each group has an id and a type, where the type of a group can either be \textit{and}, \textit{or} or \textit{xor/alternative}.
...
\todo{in the most basic form, eps are a feature models coupled with time points ordered in a list. Not many additional restraints in addition to the feature model definition.} 
