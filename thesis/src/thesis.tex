...
dra leser tilbake inn fra background. contributors are replanning evolution plans. these changes use existing tools to ensure soundness. Even though they are sound individually, the merging of the changes may result in something that is not. 

the context. we will say darwin spl. we will show screenshot. tool used in the backend.

problem definition. hva er det vi prøver å gjøre. blabla three-way fordi det er bedre. vi vil bevare structure og semantics.

the algorithm. how it works. outline. the different parts. the types of conflicts. bla bla

formelle definisjoner av evolution plans. snakke om well formedness. important prerequisite to know the formal semantics in order to do an effective merge.

visualization deviates from common stuff. highlight that groups are nodes very similarly to features. This is not important for a user, but an important aspect of the logical framework. snakke om hvordan det er visualisert. grupper er mer tydelig enkodet. osv

\todo{høynivå opplegg. outline opp hit. }

\todo{problem definition. write about this somehow?}
% \section{Problem Definition}%
% \label{sec:problem_definition}

\todo{section somewhere: about the darwin spl tool. screenshot. about the merge tool working as a backend for this tool, potentially being tied in. user should maybe not interact directly with this tool. etc}

\chapter{Implementation of the Merger in Haskell}%
