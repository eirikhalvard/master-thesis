...
begynne med å introdusere background. fortelle hva vi skal gå gjennom. fortelle om spl. div merge teknikker. haskell formalisering. etc
...
\todo{dra leser tilbake inn fra background. contributors are replanning evolution plans. these changes use existing tools to ensure soundness. Even though they are sound individually, the merging of the changes may result in something that is not.} 
...
% The common evolution plan, called \textit{base}, will implicitly provide information about what things were added, removed and changed in each of the derived evolution plans.
...
\todo{the context. we will say darwin spl. we will show screenshot. tool used in the backend. section somewhere: about the darwin spl tool. screenshot. about the merge tool working as a backend for this tool, potentially being tied in. user should maybe not interact directly with this tool. etc}

\section{Algorithm Overview}%
\label{sec:algorithm_overview}

In this section we will describe the general outline of the merge algorithm. This includes the different phases involved, as well as the potential conflicts that could occur.
...
\todo{formelle definisjoner av evolution plans. snakke om well formedness. important prerequisite to know the formal semantics in order to do an effective merge.}
...
\todo{visualization deviates from common stuff. highlight that groups are nodes very similarly to features. This is not important for a user, but an important aspect of the logical framework. snakke om hvordan det er visualisert. grupper er mer tydelig enkodet. osv}

\chapter{Implementing the Merger in Haskell}%
\label{cha:implementation_of_the_merger_in_haskell}

\section{TING FRA KAP 3}%
\label{sec:ting_fra_kap_3}

\section{Formal Semantics of Feature Model Evolution Plans}%
\label{cha:formal_semantics_of_feature_model_evolution_plans}
\todo{chapter written to a section. do something meaningful}

As stated in \textbf{RQ1} in section~\vref{sec:research_questions}, we want design the merger in a way that guarantees a sound merge result. In order to do so, we need to have a clear, formal definition of the software artifact at hand, namely feature model evolution plans.

We have defined the formal structure and semantics of evolution plans in a paper published in the Software Product Line Conference (SPLC) \cite{cite:consistency_preserving_evolution_planning}. In the paper we defined the evolution plan as an initial feature model, together with an user defined list of planning sections. Each planning section contains all operations that are scheduled for the same time point. The operations include addition and deletion of features and groups, as well as modifications to their fields, such as names or types. For every operation, we defined what makes an operation valid or invalid according to the formal definitions of feature models. 

In the work in this thesis, I will build upon the definitions and semantics of evolution plans defined in \cite{cite:consistency_preserving_evolution_planning}. The work we did defined the theoretical foundation of a sound, well-formed evolution plan, which we used to create a framework for ensuring soundness. In our merge tool, we are merging evolution plans which are assumed to be \textit{sound}. This means we can leverage the properties of sound evolution plans defined in the paper. Since we also want to create a sound evolution plan, we will create the tool in such a way that soundness is ensured.

% The semantics of editing operations were defined to create a soundness checker for evolution plans, which is used as a tool aiding replanning of evolution plans. However, since the focus of this thesis is to harmonize different alterations to evolution plan, we will need to adapt the evolution plan representation to fit this task.

\section{Issues with using a list-based approach}%
\label{sec:issues_with_using_a_list_based_approach}
\todo{create better title. maybe rewrite to be more about general issues we need to look out for. not just about the solution we have.}
...
However, when we design an approach to merging two evolution plans, the exact way each feature model in the evolution plan was derived is besides the point. The only thing the user sees are a list of feature models associated with time points. Exactly how the evolution plan was manipulated, and in what order each editing operation took place is not relevant for the user, so it should also not be relevant for the merge algorithm. As described in \textbf{RQ3} in section~\ref{sec:research_questions}, we want to produce merge results that are predictable and transparent. Having a list-based approach might make it hard to differentiate equal plans. \todo{have a small example here} Since there often is more than one way of going from one feature model to another, seemingly equal evolution plans might be represented differently.

\todo{rearrange out of place. maybe rewrite this whole thing. do something about it}
...
In order to avoid the problems stated in section \vref{sec:issues_with_using_a_list_based_approach}, we construct a new representation for evolution plans. This representation is more aligned with what the user interacts \todo{what the user interacts with. litt rart. har ikke snakket om user} with, so equal plans \todo{equal plans? hva er dette? har ikke snakket om} are also represented equally. In this representation, called \texttt{TreeUserEvolutionPlan}, the evolution plan is represented as a list of feature models together with a time point. Instead of storing the operations necessary to go from the previous time point to the next, each feature model is explicitly encoded. Each feature model is represented as a mutually recursive tree structure of features and groups. This representation is the normal form of evolution plans, and the closest representation to what the user actually interacts with.
...
\todo{text before this point. what are we jumping into?}
\todo{better naming schemes. too techniqal}
\todo{better naming for user. not really about user. meeen vi får se.}
...
\todo{refactoring? har ikke snakket om dette.}
...
\section{TING FRA RESTEN}%
\label{sec:ting_fra_resten}

\todo{fikse heading problemer. prefikse alt med sub eller no. her må man finne ut hva greia blir}
...
\todo{bla bla about tools. cli and visualization tool.}
