...
We will in this section give a rough time complexity analysis of the three-way merge algorithm. How the algorithm will perform is highly dependent on a number of factors, including the total amount of features and groups, the amount of modifications between each time point as well as the similarities between the versions. To keep it simple, we will perform a worst-case time complexity analysis with regards to the amount of time points, $t$, and the average amount of features and groups per time point, $n$
