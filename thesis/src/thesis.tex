...
We have chosen a merging strategy that is incremental. This means that we will merge one time point at a time, creating a list of feature models. Merging modifications in earlier time points might affect modifications later on in the plan. However, if a modification is not compatible with another later in the plan, we will not know this until we try to merge the latter. 

\todo{something about inspecting the evolution plan manually to determine the cause}
